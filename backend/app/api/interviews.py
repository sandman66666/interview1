...
from ..core.security import generate_interview_url, create_access_token, verify_interview_token
...
# Move token endpoint to the top with a more specific path
...
async def create_interview(
    ...
    try:
        ...
        # Create interview with a simple URL ID
        interview = models.Interview(
            id=interview_id,
            url_id=str(interview_id),  # Store just the ID as url_id
        ...
        # Generate JWT token URL for the response
        token = create_access_token(
            data={"interview_id": str(interview.id)},
            expires_delta=timedelta(days=7)
        )
        
        # Update the url_id in the response with the full JWT URL
        interview.url_id = f"{settings.FRONTEND_URL}/interview/{token}"
